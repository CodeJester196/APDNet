...
class Model(nn.Module):
    def __init__(self,  config=None):
        ...
        self.end_conv = nn.Conv2d(config.d_model,config.d_model,3,2,1)
        self.fc1 = nn.Sequential(
            nn.Linear(config.enc_in * config.seq_len, config.num_class),
        )
    ...
    def forward(self, x_enc,padding_mask):
        ...
        dec_out = dec_out + \
                  (means[:, 0, :].unsqueeze(1).repeat(
                      1, self.pred_len + self.seq_len, 1))
        dec_out = self.fc1(dec_out.reshape(dec_out.shape[0], -1))
        return dec_out
